...
Hence, the key factors impacting on the total IP score and the emotion/concern/consequences IP component at one year after COVID-19 (`r an$cmm_variables$ipq_total %>% exchange(dict = globals$var_lexicon, value = "label_long") %>% de_capitalize %>% collapse_and`) were defined as variables with non-zero coefficients in all multi-parameter models. 
...
flextable::flextable(suppl_tbl$modeling_vars) %>% 
  ...
  footnote(1, 1, 
           value = as_paragraph("CVD: cardiovascular disease; #: number of; CKD: chronic kidney disease; GID: gastrointestinal disease; PSS: persistent somatic symptoms; CFS: Chalder's fatigue score; SMWD: six-minute walking distance; SMWD vs ref.: Difference of SMWD and the expected reference SMWD value; LFT: lung function testing; CT: computed tomography of the chest; HB: blood hemoglobin; FT: serum ferritin; TF-Sat: transferrin saturation; sTFR: serum soluble transferrin receptor; NT-proBNP: serum N-terminal pro-brain natriuretic peptide; CRP: serum C-reactive protein; HbA1c: glycated blood Hb"), 
           part = 'header', 
           ref_symbols = 'a') %>% 
  theme_vanilla
...
flextable::flextable(suppl_tbl$excluded) %>% 
  ...
  footnote(1, 1, 
           value = as_paragraph('WHO COVID-19 severity: WHO ordinal scale for clinical improvement; SMWD: Six minute walking distance; Hb: Blood hemoglobin; FT: serum ferritin'), 
           ref_symbols = 'a', 
           part = 'header') %>% 
  footnote(1, 4:5, 
...
flextable::flextable(suppl_tbl$fup) %>% 
  ...
  footnote(1, 1, 
           value = as_paragraph("PSS: persistent somatic symptoms; ECOG: Eastern Cooperative Oncology Group performance score; mMRC: Modified Medical Research Council dyspnea score; CFS: Chalder's fatigure score; LFT: lung function testing; CT: computed tomography of the chest; SMWD: Six minute walking distance; SMWD vs reference: SMWD, difference between the actual and reference value"), 
           part = 'header', 
           ref_symbols = 'a') %>% 
  footnote(1, 6:7, 
...
flextable::flextable(suppl_tbl$biochemistry_fup) %>% 
  ...
  width(7, width = 2.1, unit = 'cm') %>%
  footnote(1, 1, 
           value = as_paragraph('Hb: blood hemoglobin; FT: serum ferritin; TF-Sat: serum transferrin saturation; sTFR: serum soluble transferrin receptor; NT-pro-BNP: serum N-terminal pro-brain natriuretic peptide; CRP: serum C-reactive protein; HbA1c: glycated blood Hb'), 
           part = 'header', 
           ref_symbols = 'a') %>% 
  footnote(1, 6:7, 
           value = as_paragraph('Categorical variables: \u03C7\u00B2 test with Cramer V effect size statistic, numeric variables: Kruskal-Wallis test with \u03B7\u00B2 effect size statistic'), 
...
flextable::flextable(suppl_tbl$gender) %>% 
  ...
  width(5, width = 2.1, unit = 'cm') %>% 
  footnote(1, 1, 
           value = as_paragraph('WHO COVID-19 severity: WHO ordinal scale for clinical improvement; PSS: persistent somatic symptoms; mMRC: Modified Medical Research Council dyspnea score; CT: computed tomography of the chest; Hb: blood hemoglobin; FT: serum ferritin; BIPQ: brief illness perception questionnaire'), 
           part = 'header', 
           ref_symbols = 'a') %>% 
  footnote(1, 4:5, 
           value = as_paragraph('Categorical variables: \u03C7\u00B2 test with Cramer V effect size statistic, numeric variables: Mann-Whitney test with r effect size statistic'), 
...
flextable::flextable(suppl_tbl$respi) %>% 
  ...
  width(5, width = 2.1, unit = 'cm') %>% 
  footnote(1, 1, 
           value = as_paragraph("CFS: Chalder's fatigue score; BIPQ: brief illness perception questionnaire"), 
           ref_symbols = 'a', 
           part = 'header') %>% 
  footnote(1, 4:5, 
           value = as_paragraph('Categorical variables: \u03C7\u00B2 test with Cramer V effect size statistic, numeric variables: Mann-Whitney test with r effect size statistic'), 
...
flextable::flextable(suppl_tbl$ipq_clusters) %>% 
  ...
  footnote(1, 1, 
           value = as_paragraph("WHO COVID-19 severity: WHO ordinal scale for clinical improvement; PSS: persistent somatic symptoms; ECOG: Eastern Cooperative Oncology Group performance score; mMRC: Modified Medical Research Council dyspnea score; CFS: Chalder's Fatigure Score"), 
           part = 'header', 
           ref_symbols = 'a') %>% 
  footnote(1, 5:6, 
           value = as_paragraph('Categorical variables: \u03C7\u00B2 test with Cramer V effect size statistic, numeric variables: Kruskal-Wallis test with \u03B7\u00B2 effect size statistic'), 
